...
describe('秋持ちテンパイでのUI判定呼び出しがshanを動かさないか', () => {
  ...
  // [2026-07-21 秋ドラ根治] 表示クランプ [displayBaopai/commitDoraReveal]。
  // 採点は _baopai [全量] を使うが、表示は確定枚数までしか出さない。
  // どの経路の未 commit めくりも局中に表示されない [上がりまで表示増えない]。
  it('displayBaopai は金北modal中[上がり未確定]は不変、和了確定で増える', () => {
    game.reset();
    const s: any = get(game);
    const g = s.game;
    const cur = g.lunbanToPlayerId(g.state.lunban);
    g.huapai[cur] = ['f3'];
    g.nukidoraGold[cur] = 1;
    g.lizhi.add(cur);
    const sp = buildShoupai(['p1', 'p2', 'p3', 'p4', 'p5', 'p6', 's7', 's8', 's9', 'z1', 'z1', 'z1', 'm5']);
    sp.zimo('m5');
    g.shoupai.set(cur, sp);
    s.lastZimo = 'm5';
    if ((g.shan as any)._pai.length < 4) (g.shan as any)._pai.push('m3', 's5', 'p8', 'z2');
    const dispBefore = g.shan.displayBaopai.length;
    game.tsumo();
    const mid: any = get(game);
    expect(mid.pendingKinpei).not.toBeNull();
    expect(mid.game.shan.displayBaopai.length).toBe(dispBefore);
    game.selectKinpei('aki');
    const done: any = get(game);
    expect(done.game.shan.displayBaopai.length).toBeGreaterThanOrEqual(dispBefore);
    // 採点用 _baopai は全量 [表示クランプは採点に無影響]
    expect(done.game.shan.baopai.length).toBeGreaterThanOrEqual(done.game.shan.displayBaopai.length);
  });

  it('カンドラは即 commit されて displayBaopai に出る', () => {
    game.reset();
    const s: any = get(game);
    const g = s.game;
    const cur = g.lunbanToPlayerId(g.state.lunban);
    ...
    const repl = g.declareKan(cur, 'p1111');
    if (repl) {
      expect(g.shan.displayBaopai.length).toBeGreaterThan(dispBefore);
      expect(g.shan.displayBaopai.length).toBe(g.shan.baopai.length);
    }
  });
});
